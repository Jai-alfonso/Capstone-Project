...
export default function ProfilePage() {
  ...
  const fullUserProfile = userProfile as FullUserProfile | null;
  ...
  useEffect(() => {
    if (fullUserProfile) {
      setProfileData({
        ...
        emergencyContact: fullUserProfile.emergencyContact || "",
        notes: fullUserProfile.notes || "",
      });

      setIs2FAEnabled(fullUserProfile.twoFactorEnabled || false);
    }
  }, [fullUserProfile]);
  ...
  const handleConfirmCurrentPassword = async () => {
    if (!currentPassword || !fullUserProfile?.email) {
    ...
    try {
      ...
      sessionStorage.setItem(
        `password_change_${fullUserProfile.email}`,
        JSON.stringify({
      ...
      const clientName = fullUserProfile.firstName
        ? `${fullUserProfile.firstName} ${
            fullUserProfile.lastName || ""
          }`.trim()
        : "User";
  ...
  const handleVerifyPasswordCode = async () => {
    ...
    try {
      ...
      sessionStorage.removeItem(`password_change_${fullUserProfile!.email}`);
  ...
  const handleSendPasswordResetEmail = async () => {
    ...
    try {
      ...
      const actionCodeSettings = {
        url: `${window.location.origin}/dashboard/client/profile?passwordResetSuccess=true`,
      ...
      setPasswordChangeSuccess(
        <div className="space-y-3">
          ...
          <p className="text-sm">
            ...
            <strong className="ml-1">{fullUserProfile.email}</strong>
          </p>
  ...
  const handleSave = async () => {
    ...
    try {
      const userDocRef = doc(db, "users", fullUserProfile.uid);
      ...
      if (fullUserProfile.refresh) {
        fullUserProfile.refresh();
      }
    } catch (error: any) {
  ...
  const handleEnable2FA = async () => {
    ...
    try {
      ...
      sessionStorage.setItem(
        `2fa_verification_${fullUserProfile.email}`,
        JSON.stringify({
          code: verificationCode,
          expiresAt: Date.now() + 30 * 60 * 1000,
          email: fullUserProfile.email,
  ...
  const handleVerify2FACode = async () => {
    ...
    try {
      ...
      setVerificationCode("");
      sessionStorage.removeItem(`2fa_verification_${fullUserProfile!.email}`);
                ...
                <CardDescription className="space-y-2">
                  <div className="flex items-center justify-center gap-2">
                    ...
                    <span className="text-sm">
                      {fullUserProfile.accountStatus || "active"}
                    </span>
                  </div>
                  <div className="flex items-center justify-center gap-2">
                    <Calendar className="h-4 w-4" />
                    <span className="text-sm">
                      Client since {formatDate(fullUserProfile.createdAt)}
                    </span>
                  </div>
                </CardDescription>
              </CardHeader>
                ...
                <div className="pt-4 border-t">
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span className="font-medium">
                      {formatDate(fullUserProfile.lastLoginAt)}
                    </span>
                  </div>
            ...
            <div className="flex items-center justify-between p-4 border rounded-lg">
              ...
              {is2FAEnabled ? (
                ...
                >
                  ...
                  {sendingVerification ? "Enabling..." : "Enable 2FA"}
                </Button>
              )}
            </div>
